...
public class EmailAuthManager implements AuthService {
	...
	public EmailAuthManager(AuthDao authDao,UserService userService) {
		super();
	...
	public boolean login(String email, String password) {
		if (authDao.login(email, password)) {
			Users.userList.forEach(u -> {
				if (u.getEmail() == email) {
					System.out.println("E-Posta ile giriþ baþarýlý. Hoþgeldiniz " + u.getFirstName() + " " + u.getLastName());
				}
			});
			return true;
		} else {
			System.out.println("E-Posta ile giriþ baþarýsýz. Lütfen bilgileri kontrol ediniz.");
			return false;
		}
